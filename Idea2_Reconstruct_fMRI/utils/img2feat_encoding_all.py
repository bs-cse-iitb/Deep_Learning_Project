...
def main():
    ...
    os.makedirs(f'../../encoding/z/', exist_ok=True)
    os.makedirs(f'../../encoding/c/', exist_ok=True)
    os.makedirs(f'../../encoding/zc/', exist_ok=True)
    ...
    for s in tqdm(range(imgidx[0],imgidx[1])):
        ...
        z_enc = z_enc.cpu().detach().numpy().flatten()
        c = c.cpu().detach().numpy().flatten()
        #samples = samples.cpu().detach().numpy().flatten()

        
        np.save(f'../../encoding/z/{s:06}.npy',z_enc)
        np.save(f'../../encoding/c/{s:06}.npy',c)
        #np.save(f'../../encoding/zc/{s:06}.npy',samples)
